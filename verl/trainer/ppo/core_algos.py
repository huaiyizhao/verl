...
#   - pos_slope (diff ~ slope*position) != 0   -> attention/rope drift across the long multimodal ctx
...
def _maybe_debug_logprob_gap(
    ...
    with torch.no_grad():
        ...
        # temperature signature: diff ~ (T_rollout/T_train - 1) * logp  =>  a non-zero slope on logp
        temp_slope = _lin_slope(vroll, vd)

        # position within each row's response (0-based over valid tokens), for first-token & drift checks
